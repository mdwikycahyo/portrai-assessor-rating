...
import { ParticipantKeyActionAssessment } from "../components/ParticipantKeyActionAssessment"
...
export default function RatingPage() {
  ...
  const [activeView, setActiveView] = useState<'simulation' | 'key-action-rating'>('simulation')

  // State for tracking key action completion progress
  const [keyActionCompletionStatus, setKeyActionCompletionStatus] = useState<{ [keyActionId: string]: boolean }>({})
  ...
  const handleViewChange = (view: 'simulation' | 'key-action-rating') => {
    setActiveView(view)
  }

  // Calculate progress data for all key actions across simulations
  const progressData = useMemo(() => {
    const allKeyActionIds = new Set<string>()

    // Collect all unique key action IDs from all simulations
    simulations.forEach(simulation => {
      Object.keys(simulation.keyActionRatings).forEach(keyActionId => {
        allKeyActionIds.add(keyActionId)
      })
    })

    const total = allKeyActionIds.size
    const completed = Object.values(keyActionCompletionStatus).filter(Boolean).length

    return { completed, total }
  }, [simulations, keyActionCompletionStatus])
          ...
          <div className="flex-1 transition-all duration-300">
            <div className="bg-white border border-gray-200 rounded-lg p-6 h-full overflow-y-auto">
              {activeView === 'simulation' ? (
                ...
                /* Key Action Rating View */
                <ParticipantKeyActionAssessment
                  simulations={simulations}
                  participantId={participantId}
                  completionStatus={keyActionCompletionStatus}
                  onCompletionStatusChange={setKeyActionCompletionStatus}
                />
              )}
            </div>
          </div>
